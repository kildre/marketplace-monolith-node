...
describe('MarketplaceUserDAO', () => {
  ...
  beforeEach(() => {
    (MarketplaceUser.findOne as any).mockReset?.();
    (MarketplaceUser.findByPk as any).mockReset?.();
  ...
  test('addRoles uses $add with found roles', async () => {
    ...
    await dao.addRoles(1, [10, 11]);
    expect(MarketplaceUser.findByPk).toHaveBeenCalledWith(1, { transaction: undefined });
    expect(Role.findAll).toHaveBeenCalledWith({ where: { id: [10, 11] }, transaction: undefined });
    expect(instance.$add).toHaveBeenCalledWith('roles', [{ id: 10 }, { id: 11 }], { transaction: undefined });
  });

  test('removeRole uses $remove when found', async () => {
    const instance: any = new (MarketplaceUser as any)();
    instance.$remove = jest.fn();
    (MarketplaceUser.findByPk as any).mockResolvedValue(instance);
    (Role.findByPk as any).mockResolvedValue({ id: 10 });

    await dao.removeRole(1, 10);
    expect(instance.$remove).toHaveBeenCalledWith('roles', { id: 10 }, { transaction: undefined });
  });
});
